...
@section('content')
    ...
    <h4 class="mb-4 py-3">
        <span class="text-muted fw-light">Master Video /</span> Tambah
    </h4>
    ...
    <div class="card mb-4">
        <form id="form1" class="card-body" action="{{ route('mastervideo.store') }}" method="POST"
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="source_type" id="source_type" class="form-control" required>
                        <option value="banner" {{ old('source_type') == 'banner' ? 'selected' : '' }}>Banner Utama</option>
                        <option value="youtube" {{ old('source_type') == 'youtube' ? 'selected' : '' }}>YouTube</option>
                        <option value="instagram" {{ old('source_type') == 'instagram' ? 'selected' : '' }}>Instagram
                        </option>
                        <option value="tiktok" {{ old('source_type') == 'tiktok' ? 'selected' : '' }}>TikTok</option>
                    </select>
                </div>
                ...
                <div class="col-md-12 d-none" id="video_field_group">
                    <label class="form-label" for="video">Upload File Video</label>
                    <input type="file" id="video" class="form-control" name="video" accept="video/*">
                    <small class="text-muted">Gunakan file MP4, MOV, AVI, MKV, WEBM. Max 100MB. Jika gunakan YouTube, tidak
                        perlu upload file.</small>
                </div>

                <div class="col-md-12">
                    <label class="form-label" for="deskripsi">Deskripsi</label>
                    <textarea class="form-control" id="deskripsi" name="deskripsi" rows="6">{{ old('deskripsi') }}</textarea>
                </div>
