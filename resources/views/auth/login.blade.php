...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('assets/login/css/index.css') }}" type="text/css" media="all">
...
<body>
    ...
    <div class="login-container">
        ...
        <form method="POST" class="form login" action="{{ route('log-adm') }}">
            @csrf

            <!-- Menampilkan pesan error di samping input -->
            @if ($errors->has('email'))
            ...
            <div class="form__field form__field--email">
                ...
                <div class="form__input-wrapper">
                    <input id="login__email" type="text" name="email"
                        class="form__input @if ($errors->has('email')) input-error @endif" placeholder="Email"
            ...
            <div class="form__field form__field--password">
                ...
                <div class="form__input-wrapper">
                    <input id="login__password" type="password" name="password"
                        class="form__input @if ($errors->has('password')) input-error @endif"
            ...
            <div class="form__field form__field--remember d-flex align-items-center mt-3">
                <input class="form-check-input me-2" type="checkbox" name="remember" id="remember"
                    {{ request()->cookie('remember_2fa') === 'true' ? 'checked' : '' }}>
            ...
            @php
                ...
                // ✅ HARUS begini — pakai objek, bukan static
                $crawler = new CrawlerDetect();

                // ✅ Gunakan metode objek, bukan ::
                $isCrawler = $crawler->isCrawler($agent);

                // Hasil akhir: kalau bot tampil 'Bot Detected', kalau tidak tampil nama browser
                $browser = $isCrawler ? 'Bot Detected' : $agent;

                // (Opsional) Deteksi lokasi pakai geoip kalau tersedia
                $city = $country = null;
                try {
                ...
                // 🔍 Ambil aktivitas login terakhir dari log file
                // $lastLogin = null;

                // try {
                //     $logFile = storage_path('logs/laravel-2025-10-20.log');

                //     if (File::exists($logFile)) {
                //         $lines = collect(File::lines($logFile))->reverse(); // baca dari bawah
                //         foreach ($lines as $line) {
                //             if (str_contains($line, $ip) && str_contains($line, old('email'))) {
                //                 preg_match('/\[(.*?)\]/', $line, $matches); // ambil waktu dari format [2025-10-16 10:00:00]
                //                 $lastLogin = $matches[1] ?? null;
                //                 break;
                //             }
                //         }
                //     }
                // } catch (\Throwable $e) {
                // }
                $lastLogin = showLastLoginInfo(old('email'), $ip);
            @endphp

            <div class="user-note text-muted small rounded-3 bg-light mt-4 border p-2">
                <strong>Catatan Login:</strong><br>
                📍 IP: <code>{{ $ip }}</code><br>
                🌐 Lokasi: <code>{{ $city ?? 'Tidak diketahui' }}, {{ $country ?? '' }}</code><br>
                🧭 Browser: <code>{{ $browser }}</code><br>
                🕒 Aktivitas login terakhir:
                <code>
                    {{ $lastLogin ? Carbon::parse($lastLogin)->timezone('Asia/Jakarta')->translatedFormat('d M Y, H:i') : 'Belum ada data login' }}
                </code>
            </div>
    ...
    <script src="{{ asset('assets/login/js/cartoon.js') }}"></script>
</body>
<!-- //Body -->
