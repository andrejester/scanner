...
class TabunganCBS {
	...
	static function Mutasi(){
		...
		if($vaValid['valid']){
			$cField = "Faktur,Rekening,KodeTransaksi,Keterangan,Debet,Kredit,UserName,CabangEntry,DateTime" ;
			$vaData = objData::ABrowse("mutasitabungan",$cField,"Tgl = '".self::$vaRequest['TGL']."'","","ID","ID");
		...
		if($vaValid['valid']){
			...
			if($lStatus){
				...
				foreach($vaData['data_cadangan'] as $key=>$value){
					$vaData['data_cadangan'][$key]['Saldo'] = ($vaData['data_cadangan'][$key]['StatusKaryawan']) ? "###RAHASIA###" : $vaData['data_cadangan'][$key]['Saldo'] ;
					$vaData['data_cadangan'][$key]['SaldoBlokir'] = ($vaData['data_cadangan'][$key]['StatusKaryawan']) ? "###RAHASIA###" : $vaData['data_cadangan'][$key]['SaldoBlokir'] ;
				}
  ...
  static function Log(){ 
		...
		if($vaValid['valid']){
			...
			$dTgl   	 = self::$vaRequest['TGL'] ;
			$cRekening = self::$vaRequest['REKENING'] ; 
			$dbData 	 = objData::Browse("$dbLog.tabungan","*","Tgl <= '$dTgl' and Rekening='$cRekening'","","","DateTime DESC");
