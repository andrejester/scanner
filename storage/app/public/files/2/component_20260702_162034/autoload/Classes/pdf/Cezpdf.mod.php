...
class Cezpdf extends Cpdf {
	...
	/*
	Variable Customer Assist
	*/
	public $nPageHeaderLine = 0 ;
	public $nExportCSV = 0 ;
	public $cFileCSV = "" ;
	public $vaHeader = array() ;
  private $cDirCSV = "/var/www/prg/app/data-integrasi";
	function __construct($paper='LETTER',$orientation='portrait',$pagenumber='1',$nPaperIncWidth=0,$nPaperIncHeight=0){
		$vaPage = PrintDialog::Get() ;
		$nTopMargin = $vaPage["mtop"]/10 ;
		$nLeftMargin = $vaPage["mleft"]/10 ;
		$nBottomMargin = $vaPage["mbottom"]/10 ; ;
		$nRightMargin = $vaPage["mright"]/10 ; ;
		$this->nExportCSV = $vaPage["export"] ; ;

		$nPaperIncWidth = $vaPage["pwidth"] ;
		$nPaperIncHeight = $vaPage["pheight"] ;
	...
	function ezStream($options=''){
		$this->vaHeader = array_slice($this->vaHeader,-4) ;
		$va = array_replace_recursive($_POST,$this->vaHeader);
		SysLog::Report("09",$va) ;
		if(empty($this->nExportCSV)){
			...
			$cLink = str_replace($this->cDirCSV."/tmp","/tmp-csv",$this->cFileCSV) ;
			echo('<a href="' . $cLink . '">Click Disini</a> Untuk Downloads File CSV') ;
	...
	function ezTable(&$data,$cols='',$title='',$options=''){
		...
		if (!is_array($cols)){
			reset($data);    
			list($k,$v)=[key($data), current($data)];
	...
	*/
	...
	{
		$cDir = Svr::GetDocumentRoot() . "/tmp"; //"../tmp";
		$cDir = $this->cDirCSV . "/tmp"; //Svr::GetDocumentRoot() ."/tmp" ; //"../tmp";
		...
		if (!is_dir($cDir)) {
			mkdir($cDir);
			file_put_contents($cDir . "/index.php", "");
		}
