...
class Cpdf {
	...
	protected $nStateStack = 0 ;
	protected $numPages = 0 ;
	protected $stack = [] ;
	protected $nStack = 0 ;
	protected $looseObjects = [] ;
	protected $addLooseObjects = [] ;
	protected $infoObject = 0 ;
	protected $numImages = 0 ;
	protected $options = ['compression'=>1] ;
	protected $firstPageId ;
	protected $wordSpaceAdjust = 0;
	protected $procsetObjectId ;
	protected $fontFamilies = [];
	protected $currentTextState = ''; 
	protected $messages = '';
	protected $arc4 = '';
	protected $arc4_objnum = 0;
	protected $fileIdentifier = '';
	protected $encrypted = 0;
	protected $encryptionKey = '';
	protected $callback = [];
	protected $nCallback = 0;
	...
	public function dirFont($cFontName){
		$cDir = realpath(__DIR__) . "/fonts/" . $cFontName ;
		return $cDir ;
	}
		...
		switch ($action){
			...
			case 'viewerPreferences':
				if (!isset($this->objects[$id]['info']['viewerPreferences'])){
					...
					$this->objects[$id]['info']['viewerPreferences']=$this->numObj;
				}
				$vp = $this->objects[$id]['info']['viewerPreferences'];
				...
				foreach($this->objects[$id]['info'] as $k=>$v){
					switch($k){
						...
						case 'pages':
							...
							break;
						case 'viewerPreferences':
							$res.="\n".'/ViewerPreferences '.$this->objects[$id]['info']['viewerPreferences'].' 0 R';
							break;
						case 'openHere':
							$res.="\n".'/OpenAction '.$this->objects[$id]['info']['openHere'].' 0 R';
							break;
					}
				}
				$res.=" >>\nendobj";
			...
			case 'page':
				if (!is_array($options)){
					...
					if (isset($options['id']) && isset($options['rid']) && isset($options['pos'])){
						$i = array_search($options['rid'],$this->objects[$id]['info']['pages']);
						if (isset($this->objects[$id]['info']['pages'][$i]) && $this->objects[$id]['info']['pages'][$i]==$options['rid']){
							...
							if ($k>=0){
								for ($j=count($this->objects[$id]['info']['pages'])-1;$j>=$k;$j--){
									$this->objects[$id]['info']['pages'][$j+1]=$this->objects[$id]['info']['pages'][$j];
								}
								$this->objects[$id]['info']['pages'][$k]=$options['id'];
							}
						}
					} 
				}
	...
	public function o_font($id,$action,$options=''){
		switch ($action){
			...
			case 'add':
				...
				break;
			case 'out':
				$res="\n".$id." 0 obj\n<< /Type /Font\n/Subtype /".$this->objects[$id]['info']['SubType']."\n";
				$res.="/Name /F".$this->objects[$id]['info']['fontNum']."\n";
				$res.="/BaseFont /".$this->objects[$id]['info']['name']."\n";
				if (isset($this->objects[$id]['info']['encodingDictionary'])){
					// then place a reference to the dictionary
					$res.="/Encoding ".$this->objects[$id]['info']['encodingDictionary']." 0 R\n";
				} else if (isset($this->objects[$id]['info']['encoding'])){
					// use the specified encoding
					$res.="/Encoding /".$this->objects[$id]['info']['encoding']."\n";
				}
				if (isset($this->objects[$id]['info']['FirstChar'])){
					$res.="/FirstChar ".$this->objects[$id]['info']['FirstChar']."\n";
				}
				if (isset($this->objects[$id]['info']['LastChar'])){
					$res.="/LastChar ".$this->objects[$id]['info']['LastChar']."\n";
				}
				if (isset($this->objects[$id]['info']['Widths'])){
					$res.="/Widths ".$this->objects[$id]['info']['Widths']." 0 R\n";
				}
				if (isset($this->objects[$id]['info']['FontDescriptor'])){
					$res.="/FontDescriptor ".$this->objects[$id]['info']['FontDescriptor']." 0 R\n";
				}
				$res.=">>\nendobj";
				return $res;
				break;
	...
	public function o_fontDescriptor($id,$action,$options=''){
		switch ($action){
			...
			case 'out':
				$res="\n".$id." 0 obj\n<< /Type /FontDescriptor\n";
				foreach ($this->objects[$id]['info'] as $label => $value){
					switch ($label){
						case 'Ascent':
						case 'CapHeight':
						case 'Descent':
	...
	public function o_fontEncoding($id,$action,$options=''){
		switch ($action){
			...
			case 'out':
				$res="\n".$id." 0 obj\n<< /Type /Encoding\n";
				if (!isset($this->objects[$id]['info']['encoding'])){
					$this->objects[$id]['info']['encoding']='WinAnsiEncoding';
				}
				if ($this->objects[$id]['info']['encoding']!='none'){
					$res.="/BaseEncoding /".$this->objects[$id]['info']['encoding']."\n";
				}
				$res.="/Differences \n[";
				$onum=-100;
				foreach($this->objects[$id]['info']['differences'] as $num=>$label){
					if ($num!=$onum+1){
		...
		switch ($action){
			...
			case 'add':
				...
				switch ($options) {
					...
					case 'ImageC':
					case 'ImageI':
						$this->objects[$id]['info'][$options]=1;
						break;
				}
				break;
			case 'out':
				$res="\n".$id." 0 obj\n[";
				foreach ($this->objects[$id]['info'] as $label=>$val){
					$res.='/'.$label.' ';
				}
				$res.="]\nendobj";
				return $res;
	...
	public function o_info($id,$action,$options=''){
		switch ($action){
			case 'new':
				...
				$date = 'D:'. $d->format("YmdHis") ;
				$this->objects[$id] = array('t'=>'info', 'info'=>array("Creator"=>"Assist-Component",'Author'=>'PT. Assist Software Indonesia Pratama http://www.assistindo.com', 'CreationDate'=>$date,'ModDate'=>$date));
				break;
			case 'Title':
			case 'Author':
			case 'Subject':
			...
			case 'out':
				...
				switch($this->objects[$id]['type']){
					case 'ilink':
						...
						$res.="\n/S /GoTo\n/D ".$this->destinations[(string)$this->objects[$id]['info']['label']]." 0 R";
						break;
					case 'URI':
						$res.="\n/S /URI\n/URI (";
						if ($this->encrypted){
							$res.=$this->filterText($this->ARC4($this->objects[$id]['info']));
						} else {
							$res.=$this->filterText($this->objects[$id]['info']);
						}
						$res.=")";
						break;
				}
	...
	public function o_annotation($id,$action,$options=''){
		switch ($action){
			...
			case 'out':
				$res="\n".$id." 0 obj\n<< /Type /Annot";
				switch($this->objects[$id]['info']['type']){
					case 'link':
					case 'ilink':
						$res.= "\n/Subtype /Link";
						break;
				}
				$res.="\n/A ".$this->objects[$id]['info']['actionId']." 0 R";
				$res.="\n/Border [0 0 0]";
				$res.="\n/H /I";
				$res.="\n/Rect [ ";
				foreach($this->objects[$id]['info']['rect'] as $v){
					$res.= sprintf("%.4f ",$v);
				}
				$res.="]";
				$res.="\n>>\nendobj";
	...
	public function o_page($id,$action,$options=''){
		switch ($action){
			...
			case 'annot':
				...
				if (!isset($this->objects[$id]['info']['annot'])){
					$this->objects[$id]['info']['annot']=array();
				}
				// $options should contain the id of the annotation dictionary
				$this->objects[$id]['info']['annot'][]=$options;
				break;
			case 'out':
				$res="\n".$id." 0 obj\n<< /Type /Page";
				...
				if (isset($this->objects[$id]['info']['annot'])){
					$res.="\n/Annots [";
					foreach($this->objects[$id]['info']['annot'] as $aId){
						$res.=" ".$aId." 0 R";
					}
					$res.=" ]";
				}
	...
	public function o_encryption($id,$action,$options=''){
		switch($action){
			...
			case 'out':
				...
				$res.="\n/V 1";
				$res.="\n/R 2";
				$res.="\n/O (".$this->filterText($this->objects[$id]['info']['O']).')';
				$res.="\n/U (".$this->filterText($this->objects[$id]['info']['U']).')';
				// and the p-value needs to be converted to account for the twos-complement approach
				$this->objects[$id]['info']['p'] = (($this->objects[$id]['info']['p']^255)+1)*-1;
				$res.="\n/P ".($this->objects[$id]['info']['p']);
				$res.="\n>>\nendobj\n";
	...
	//        $fontObj = $this->numObj;
					...
					#Penambahan untuk PHP V8
					$tmp = false ;
					if(version_compare(PHP_VERSION, '5.3.0', '<')){
						$tmp = set_magic_quotes_runtime() ;
					}
					$data = fread($fp,filesize($fbfile)) ;
					$tmp = true ;
					if(version_compare(PHP_VERSION, '5.3.0', '<')){
	...
	* This public function is to be called whenever the currentTextState is changed, it will update
	* the currentFont setting to whatever the appropriatte family one is.
	* If the user calls selectFont themselves then that will reset the currentBaseFont, and the currentFont
	* This public function will change the currentFont to whatever it should be, but will not change the 
	* currentBaseFont.
	*
	* @access private
	*/
	public function setCurrentFont(){
		if (strlen($this->currentBaseFont)==0){
			...
			$this->selectFont($this->dirFont('Helvetica.afm'));
		}
	...
	/**
	* public function for the user to find out what the ID is of the first page that was created during
	...
	*/
	public function getFontHeight($size){
		if (!$this->numFonts){
			$this->selectFont($this->dirFont('Helvetica'));
		}
	...
	*/
	...
	/**
	* wrapper public function for PRVTcheckTextDirective1
	*
	* @access private
	*/
	...
	* the location can be sent to the callback public function if required
	...
	*/
				...
				case '/':
					...
					switch($text[$j]){
						...
						case 'i':
							...
							break;
						case 'c':
							// this this might be a callback public function
							$j++;
							$k = strpos($text,'>',$j);
							if ($k!==false && $text[$j]==':'){
								...
								$directive = $k-$i+1;
								$f=0;
								// split the remainder on colons to get the public function name and the paramater
								...
								if (!isset($func) || !strlen(trim($func))){
									...
									// only call the public function if this is the final call
									if ($final){
										...
										$ret = $this->$func($info);
										if (is_array($ret)){
											// then the return from the callback public function could set the position, to start with, later will do font colour, and font
	...
	//          $bits = explode(':',substr($text,$j+1,$k-$j-1));
						...
						if (!isset($func) || !strlen(trim($func))){
							...
							// only call the public function if this is the final call, ie, the one actually doing printing, not measurement
							if ($final){
								// need to assess the text position, calculate the text width to this point
								// can use getTextWidth to find the text width I think
	...
	*/
	public function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
		if (!$this->numFonts){$this->selectFont($this->dirFont('Helvetica'));}

		// if there are any open callbacks, then they should be called, to show the start of the line
		if ($this->nCallback>0){
			for ($i=$this->nCallback;$i>0;$i--){
				// call each public function
	...
	* this can be called externally, but is alse used by the other class public functions
	*/
	public function getTextWidth($size,$text){
		// this public function should not change any of the settings, though it will need to
	...
	*/
	public function addTextWrap($x,$y,$width,$size,$text,$justification='left',$angle=0,$test=0,$lWrap=true){
	...
	* the public function will return the object number
	*/
	...
	*/
	...
	/**
	* after an object has been created, it wil only show if it has been added, using this public function.
	*/
	...
	*/
	...
	public function addJpegFromFile($img,$x,$y,$w=0,$h=0){
		...
		#diganti seperti ini agar bisa dijalankan di php 8
		$tmp = false ;
		if(version_compare(PHP_VERSION, '5.3.0', '<')){
			$tmp = set_magic_quotes_runtime() ;
		}
	...
	public function addJpegImage_common(&$data,$x,$y,$w=0,$h=0,$imageWidth=0,$imageHeight=0,$channels=3){
		$this->numImages++;
		$im=$this->numImages;
		$label='I'.$im;
		$this->numObj++;
	...
	public function openHere($style,$a=0,$b=0,$c=0){
		// this public function will open the document at a specified page, in a specified style
