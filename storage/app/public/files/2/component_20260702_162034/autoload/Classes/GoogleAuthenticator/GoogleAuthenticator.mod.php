...
class GoogleAuthenticator
{
     ...
     * @return string
     */
    static function createSecret($secretLength = 16)
    {
     ...
     * @return string
     */
    static function getCode($secret, $timeSlice = null)
    {
        if ($timeSlice === null) {
     ...
     * @return string
     */
    static function getQRCodeGoogleUrl($name, $secret, $title = null, $params = array())
    {
        $width = !empty($params['width']) && (int) $params['width'] > 0 ? (int) $params['width'] : 300;
        $height = !empty($params['height']) && (int) $params['height'] > 0 ? (int) $params['height'] : 300;
        $level = !empty($params['level']) && array_search($params['level'], array('qrl', 'qrm', 'qrq', 'qrh')) !== false ? $params['level'] : 'qrm';

        $urlencoded = urlencode('otpauth://totp/'.$name.'?secret='.$secret.'');
        if (isset($title)) {
            $urlencoded .= urlencode('&issuer='.urlencode($title));
        }
				return "https://cds.myassist.id/qrcode/?s=$level&d=$urlencoded&w=$width&h=$height";
    }
     ...
     */
    static function setCodeLength($length)
    {
